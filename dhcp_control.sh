...
# the live /var/udhcpd/udhcpd.conf content. See conversation history for
# the id->name table if this ever needs re-deriving.
#
...
# ── setup_dhcpd: regenerate udhcpd.conf from current MIB state ─────────────
setup_dhcpd() {
    ...
    # ── DNS: LAN_DHCP_DNS_OPT=1 -> DHCPS_DNS1/2/3, else -> router proxies (LAN_IP_ADDR) ──
...
"
    ...
    # ── NTP: NTP_SERVER_ID selects which NTP_SERVER_HOST<N> feeds opt ntpsrv ───
...
# ── stop/start/restart — the actual /bin/udhcpd process, -S = server mode ──
stop_dhcpd() {
    ...
    if [ -n "$pid" ] && pid_alive "$pid"; then
        ...
        # pidfile stale/missing — fall back to matching by command line
...
case "$1" in
    ...
    *)
        ...
        echo "  setup   - regenerate $DHCPD_CONF from current MIB values only" >&2
        echo "  start   - regenerate conf if missing, then launch $DHCPD_BIN -S" >&2
        echo "  stop    - stop the running daemon" >&2
        echo "  restart - stop, regenerate conf, start (use this after 'mib set'+'mib commit')" >&2
        exit 1
        ;;
esac
